...
import { Calendar, Plus, Search, Users, Trash2, Link as LinkIcon, ChevronRight } from 'lucide-react'
...
export default function EventosPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.nombre || !form.fecha || !profile) return
    setSaving(true)
    await crearEvento({ nombre: form.nombre, descripcion: form.descripcion, fecha: new Date(form.fecha) }, profile.id)
    setSaving(false)
    setShowModal(false)
  ...
  return (
    <>
      ...
      <div className="px-4 lg:px-8 py-6 max-w-6xl w-full mx-auto">
        ...
        {loading ? (
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {filtered.map(ev => (
              <div key={ev.id} className="bg-[#111113] border border-[#27272a] rounded-2xl p-5 hover:border-zinc-600 transition group relative">
                <div className="flex items-start justify-between gap-2 mb-3">
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-white truncate">{ev.nombre}</h3>
                    <p className="text-zinc-500 text-xs mt-0.5">
                      {format(toDate(ev.fecha), "d MMM yyyy", { locale: es })}
                    </p>
                  </div>
                  <div className={`w-2.5 h-2.5 rounded-full shrink-0 mt-1 ${ev.activo ? 'bg-emerald-400' : 'bg-zinc-600'}`} />
                </div>
                {ev.descripcion && (
                  <p className="text-zinc-400 text-sm line-clamp-2 mb-4">{ev.descripcion}</p>
                )}
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={() => copyLink(ev.id!)} title="Copiar link del scanner" className="p-2 rounded-lg text-zinc-500 hover:text-blue-400 hover:bg-blue-400/10 transition">
                      <LinkIcon size={15} />
                    </button>
                    {profile?.rol === 'admin' && (
                      <button onClick={() => setConfirmDel(ev.id!)} className="p-2 rounded-lg text-zinc-500 hover:text-red-400 hover:bg-red-400/10 transition">
                        <Trash2 size={15} />
                      </button>
                    )}
                    <Link href={`/dashboard/eventos/${ev.id}`} className="p-2 rounded-lg text-zinc-500 hover:text-white hover:bg-white/10 transition">
      ...
      {/* Create modal */}
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Crear nuevo evento">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={() => setShowModal(false)} className="flex-1 py-2.5 rounded-xl border border-[#27272a] text-zinc-400 text-sm hover:bg-white/5 transition">
              Cancelar
            </button>
