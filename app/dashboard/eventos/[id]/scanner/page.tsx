...
// Campos separados por \x1E (ASCII 30):
...
// ── QR / MRZ parser (cédula nueva) ───────────────────────────────────────────
// El QR de la cédula nueva contiene las 3 líneas MRZ:
// Línea 1: ICCOL...
...
//   0503291M3306149COL1077721837<6   → cédula = "1077721837"
...
function parseMrzLines(l1: string, l2: string, l3: string): Cedula | null {
  ...
  return { cedula, nombres, apellidos, sexo, modo: 'QR', ...(dob ?? {}) }
}

function parseQrMrz(raw: string): Cedula | null {
...
function parseBarcode(raw: string): Cedula | null {
  ...
  // PDF417 (cédula vieja): \x1E record separator
  if (raw.includes(RS)) return parsePdf417(raw)

  // QR / MRZ (cédula nueva): ICCOL prefix, << name separator, or COL+digits pattern
  const up = raw.toUpperCase()
  if (up.includes('ICCOL') || up.includes('IDCOL') || raw.includes('<<')) {
    return parseQrMrz(raw)
  }

  // Fallback: try PDF417 with other separators
...
// ── IScannerControls (from @zxing/browser) ────────────────────────────────────
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      await registrarAsistencia(eventoId, {
        ...
        edad,
        rh:              confirmForm.rh,
        modo:            confirmForm.modo === 'PDF417' ? 'PDF417' : 'MRZ',
      })
      ...
      setConfirmForm(null)
      setTimeout(() => { setBanner(null); reiniciarScanner() }, 3000)
    } catch (err: unknown) {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      try {
        ...
        if (typeof (controls as ScannerControls).switchTorch === 'function') {
          setHasTorch(true)
        }
      } catch {
  ...
  return (
    <div className="fixed inset-0 bg-black z-50 overflow-hidden select-none">
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        >
          {confirmForm ? 'Pausado — confirma o cancela' : 'Apunta al reverso de la cédula'}
        </p>
      </div>
        ...
        <div className="flex items-center gap-2 mt-1">
          <div
            className={`w-2.5 h-2.5 rounded-full transition-colors ${
              confirmForm ? 'bg-zinc-600' : scanning ? 'bg-emerald-400 animate-pulse' : 'bg-amber-400 animate-pulse'
            }`}
            title={confirmForm ? 'Pausado' : scanning ? 'Escaneando' : 'Iniciando cámara…'}
          />
          <button
            onClick={toggleTorch}
        ...
        <div className="flex gap-2 mb-3">
          ...
          <div className="flex-1 flex items-center justify-center py-3.5 rounded-2xl bg-white/5 border border-white/8 gap-2">
            <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${
              confirmForm ? 'bg-zinc-500' : scanning ? 'bg-emerald-400 animate-pulse' : 'bg-amber-400 animate-pulse'
            }`} />
            <span className="text-white/50 text-xs">
              {confirmForm ? 'Pausado' : scanning ? 'Apunta al reverso de la cédula' : 'Iniciando cámara…'}
            </span>
          </div>
