...
import { getEventoByToken, registrarAsistencia, checkDuplicado, registrarLog } from '../../../lib/firestore'
...
function debugPdf417Positions(raw: string): string {
...
type Screen   = 'loading' | 'notfound' | 'name' | 'select' | 'scan'
...
export default function PublicScannerPage() {
  ...
  const addLog = useCallback((m: string) => setLog(p => [...p.slice(-6), m]), [])

  // Load event by token
  useEffect(() => {
    if (!token) { setScreen('notfound'); return }
    getEventoByToken(token).then(ev => {
      if (!ev || !ev.activo) { setScreen('notfound'); return }
      setEvento(ev)
      setScreen('name')
  ...
  useEffect(() => {
    if (!evento?.id) return
    return onSnapshot(collection(db, 'eventos', evento.id, 'asistencias'), s => setTotal(s.size))
  }, [evento?.id])

  // Camera when entering scan screen
  useEffect(() => {
    ...
    return () => {
      ...
      setCameraOn(false); setFlashAvail(false); setFlashOn(false)
    }
  }, [screen])
  ...
  const processCanvas = useCallback(async (canvas: HTMLCanvasElement) => {
    ...
    addLog(`foto ${canvas.width}×${canvas.height}`)
    ...
    const dup = await checkDuplicado(evento!.id!, detected.cedula)
    ...
    setProcessing(false)
  }, [evento, scanMode, showToast, addLog, ocrFails])
  ...
  const handleConfirm = async () => {
    if (!confirmForm || !evento) return
    setConfirmSaving(true); setConfirmError('')
    try {
      const edad = confirmForm.fechaNacimiento ? calcEdad(confirmForm.fechaNacimiento) : 0
      const asistenciaId = await registrarAsistencia(evento.id!, {
        cedula: confirmForm.cedula, nombres: confirmForm.nombres, apellidos: confirmForm.apellidos,
        fechaNacimiento: confirmForm.fechaNacimiento, edad, sexo: (confirmForm.sexo || undefined) as 'M' | 'F' | undefined,
        rh: confirmForm.rh, modo: confirmForm.modo, registradoPor: operatorName,
      })
      registrarLog({ tipo: 'REGISTRO', eventoId: evento.id!, eventoNombre: evento.nombre, asistenciaId, cedula: confirmForm.cedula, nombreAsistente: `${confirmForm.apellidos} ${confirmForm.nombres}`.trim(), operadorUid: '', operadorNombre: operatorName, operadorEmail: '', detalles: `Modo: ${confirmForm.modo} (escáner público)`, ip: '' })
  ...
  const handleManual = async (e: React.FormEvent) => {
    ...
    if (!cedula || !nombres || !apellidos || !fechaNacimiento || !sexo || !evento) return
    setManualSaving(true); setManualError('')
    try {
      if (await checkDuplicado(evento.id!, cedula.trim())) { setManualError('Cédula ya registrada'); return }
      const asistenciaId = await registrarAsistencia(evento.id!, { cedula: cedula.trim(), nombres: capitalize(nombres.trim()), apellidos: capitalize(apellidos.trim()), fechaNacimiento, edad: calcEdad(fechaNacimiento), sexo: sexo as 'M' | 'F', rh: rh.trim(), modo: 'MANUAL', registradoPor: operatorName })
      registrarLog({ tipo: 'REGISTRO', eventoId: evento.id!, eventoNombre: evento.nombre, asistenciaId, cedula: cedula.trim(), nombreAsistente: `${capitalize(apellidos.trim())} ${capitalize(nombres.trim())}`.trim(), operadorUid: '', operadorNombre: operatorName, operadorEmail: '', detalles: 'Modo: MANUAL (escáner público)', ip: '' })
  ...
  if (screen === 'loading') {
    return (
      <div style={{ position: 'fixed', inset: 0, background: '#09090b', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ width: 32, height: 32, border: '3px solid rgba(34,197,94,0.2)', borderTopColor: '#22c55e', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />
  ...
  if (screen === 'notfound') {
    return (
      <div style={{ position: 'fixed', inset: 0, background: '#09090b', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, padding: 24 }}>
        <div style={{ fontSize: 48 }}>🚫</div>
        <p style={{ color: '#fff', fontWeight: 700, fontSize: 18, margin: 0 }}>Evento no encontrado</p>
        <p style={{ color: '#71717a', fontSize: 13, textAlign: 'center', margin: 0 }}>El link de acceso es inválido o el evento está desactivado.</p>
      </div>
    )
  }
  ...
  if (screen === 'name') {
    return (
      <div style={{ position: 'fixed', inset: 0, background: '#09090b', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        ...
        <div className="fadein" style={{ width: '100%', maxWidth: 380 }}>
          <div style={{ textAlign: 'center', marginBottom: 32 }}>
            <p style={{ color: '#4ade80', fontSize: 13, fontWeight: 600, margin: '0 0 6px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Control de acceso</p>
            <p style={{ color: '#fff', fontWeight: 700, fontSize: 22, margin: '0 0 4px' }}>{evento?.nombre}</p>
            {evento?.lugar && <p style={{ color: '#71717a', fontSize: 13, margin: 0 }}>{evento.lugar}</p>}
          </div>

          <div style={{ background: '#111', border: '1px solid #27272a', borderRadius: 20, padding: 24 }}>
            <p style={{ color: '#a1a1aa', fontSize: 13, margin: '0 0 16px', textAlign: 'center' }}>¿Cómo te llamas? (operador)</p>
            <input
              value={nameInput}
              onChange={e => setNameInput(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter' && nameInput.trim()) { setOperatorName(nameInput.trim()); setScreen('select') } }}
            ...
            />
            <button
              onClick={() => { if (nameInput.trim()) { setOperatorName(nameInput.trim()); setScreen('select') } }}
              disabled={!nameInput.trim()}
              style={{ width: '100%', padding: '14px 0', borderRadius: 14, border: 'none', background: nameInput.trim() ? '#16a34a' : '#27272a', color: '#fff', fontSize: 15, fontWeight: 700, cursor: nameInput.trim() ? 'pointer' : 'default', transition: 'background 0.2s' }}
            >
              Continuar →
            </button>
          </div>
    ...
    return (
      <div style={{ position: 'fixed', inset: 0, background: '#000', zIndex: 50, display: 'flex', flexDirection: 'column', userSelect: 'none' }}>
        ...
        <div style={{ background: 'linear-gradient(to bottom,rgba(0,0,0,0.9),transparent)', padding: '44px 16px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={() => setScreen('name')} style={{ width: 38, height: 38, borderRadius: '50%', background: 'rgba(255,255,255,0.1)', border: 'none', color: '#fff', fontSize: 22, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>‹</button>
          ...
          <div style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid #27272a', borderRadius: 10, padding: '4px 10px' }}>
            <p style={{ color: '#a1a1aa', fontSize: 10, margin: 0 }}>Operador</p>
            <p style={{ color: '#fff', fontSize: 11, fontWeight: 600, margin: 0 }}>{operatorName}</p>
          </div>
        </div>
        ...
        <div className="fadein" style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 24px', gap: 20 }}>
          ...
          <button onClick={() => { setScanMode('MRZ'); setScreen('scan') }}
            ...
            <div>
              ...
              <p style={{ color: '#86efac', fontSize: 12, margin: '0 0 8px', lineHeight: 1.4 }}>Emitida desde 2016 · franja MRZ en el frente</p>
    ...
    <div style={{ position: 'fixed', inset: 0, background: '#000', zIndex: 50, overflow: 'hidden', display: 'flex', flexDirection: 'column', userSelect: 'none' }}>
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 20, background: 'linear-gradient(to bottom,rgba(0,0,0,0.88),transparent)', padding: '44px 16px 20px', display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button onClick={() => { setScreen('select'); setLog([]); setScanState('idle') }} style={{ fontSize: 10, padding: '4px 10px', borderRadius: 20, fontWeight: 600, background: 'rgba(255,255,255,0.1)', border: 'none', color: '#a1a1aa', cursor: 'pointer' }}>cambiar</button>
          {scanMode === 'MRZ' && (
      ...
      <div style={{ position: 'absolute', top: '58%', left: 0, right: 0, textAlign: 'center', zIndex: 10 }}>
        <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: 11, margin: 0, padding: '0 24px' }}>
          {scanMode === 'PDF417' ? 'Enfoca el REVERSO de la cédula dentro del marco' : 'Enfoca la parte inferior del FRENTE de la cédula'}
        </p>
      </div>
